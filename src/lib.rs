...
impl Simpath {
    ...
    pub fn from_webgraph<T: RandomAccessGraph>(graph: &T) -> Self {
        ...
        Simpath {
            ...
            boundary: 0,
            head: 0,
            htable: HashMap::with_capacity(memsize),
            htid: 0,
            htcount: 0,
            wrap: 1,
            vert: Vec::with_capacity(num_nodes + 1),
            num: HashMap::with_capacity(num_nodes),
            arcto: Vec::with_capacity(graph.num_arcs().try_into().unwrap()),
            firstarc: Vec::with_capacity(num_nodes + 2),
            mate: Vec::with_capacity(num_nodes + 3),
            serial: 0,
            newserial: 0,
            log_memsize,
            memsize,
