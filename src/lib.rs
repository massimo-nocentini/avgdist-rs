...
impl Simpath {
    ...
    pub fn from_webgraph<T: RandomAccessGraph>(graph: &T) -> Self {
        ...
        Simpath {
            ...
            boundary: 0,
            head: 0,
            htable: HashMap::with_capacity(memsize >> 3),
            htid: 0,
            htcount: 0,
            wrap: 1,
            vert: Vec::with_capacity((num_nodes + 1) >> 3),
            num: HashMap::with_capacity(num_nodes >> 3),
            arcto: Vec::with_capacity((graph.num_arcs() as usize) >> 3),
            firstarc: Vec::with_capacity((num_nodes + 2) >> 3),
            mate: Vec::with_capacity((num_nodes + 3) >> 3),
            serial: 0,
            newserial: 0,
            log_memsize,
            memsize,
